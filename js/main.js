const API_URL = `https://linen-alpine-brush.glitch.me/`;
...
const startSlider = ()=> {
    ...
    const checkSlider = () => {
        if((activeSlider + 2 === sliderItems.length && document.documentElement.offsetWidth > 560) || activeSlider === sliderItems.length) {
    ...
    const nextSlide = () => {
        sliderItems[activeSlider]?.classList.remove('slider__item-active');
        position = -sliderItems[0].clientWidth * activeSlider;

        sliderList.style.transform = `translateX(${position}px)`;
        activeSlider += 1;
        sliderItems[activeSlider]?.classList.add('slider__item-active');
        checkSlider();
    };
...
const renderService = (wrapper, data) =>{
    const labels = data.map((item)=> {
        ...
        label.innerHTML = `
            <input class="radio__input" type="radio" name="service" value="${item}">
...
const renderMonth = (wrapper, data) =>{
    const labels = data.map((item) => {
        ...
        label.innerHTML = `
            ...
            ${new Intl.DateTimeFormat('ru-RU', {month: 'long'}).format(new Date(item))}</span>
`;
        return label;
    });
    wrapper.append(...labels);
...
const renderDay = (wrapper, data, month) =>{
    const labels = data.map((day)=> {
        ...
        label.innerHTML = `
            ...
            <span class="radio__label">
            ${new Intl.DateTimeFormat('ru-RU', {month: 'long', day: 'numeric'}).format(new Date(`${month}/${day}`))}</span>
`;
        return label;
    });
    wrapper.append(...labels);
